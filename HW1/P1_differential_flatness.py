...
def compute_traj_coeffs(initial_state: State, final_state: State, tf: float, n_basis: int = 4):
    """
    Inputs:
        initial_state (State)
        final_state (State)
    ...
    """
    ########## Code starts here ##########
    basis_functions = [np.polynomial.Polynomial.basis(i) for i in range(n_basis)]
    basis_matrix = np.array([[phi.deriv(d)(0) for phi in basis_functions] for d in range(2)] +
                            [[phi.deriv(d)(tf) for phi in basis_functions] for d in range(2)])
    x_coeffs = np.linalg.solve(basis_matrix,
                               np.array([initial_state.x, initial_state.xd,
                                         final_state.x, final_state.xd]))
    y_coeffs = np.linalg.solve(basis_matrix,
                               np.array([initial_state.y, initial_state.yd,
                                         final_state.y, final_state.yd]))
    coeffs = np.concatenate([x_coeffs, y_coeffs])
    # print(coeffs)
    ########## Code ends here ##########
    return coeffs


def compute_traj(coeffs, tf, N, n_basis: int = 4):
    """
    Inputs:
        coeffs (np.array shape [8]), coefficients on the basis functions
        tf (float) final_time
    ...
    ########## Code starts here ##########
    x_coeffs = coeffs[:n_basis]
    y_coeffs = coeffs[n_basis:]
    x_poly = np.polynomial.Polynomial(x_coeffs)
    y_poly = np.polynomial.Polynomial(y_coeffs)
    x_dot_poly = x_poly.deriv(1)
    y_dot_poly = y_poly.deriv(1)
    x_dot_dot_poly = x_poly.deriv(2)
    y_dot_dot_poly = y_poly.deriv(2)
    dx = x_dot_poly(t)
    dy = y_dot_poly(t)
    theta = np.arctan2(dy, dx)
    traj = np.column_stack([x_poly(t), y_poly(t), theta,
                            dx, dy,
                            x_dot_dot_poly(t), y_dot_dot_poly(t)])
    ########## Code ends here ##########
...
def compute_controls(traj):
    ...
    row_1 = np.concatenate([np.expand_dims(cos_theta, -1),
                            np.expand_dims(np.multiply(-V, sin_theta), -1)], axis=1)
    row_2 = np.concatenate([np.expand_dims(sin_theta, -1),
                            np.expand_dims(np.multiply(V, cos_theta), -1)], axis=1)
    j_matrix = np.stack([row_1, row_2], axis=1)

    ddx, ddy = traj[:, -2], traj[:, -1]
    dep = np.stack([ddx, ddy], axis=-1)
    a_w = np.linalg.solve(j_matrix, dep)
    om = a_w[:, -1]
    ########## Code ends here ##########
...
def rescale_V(V, om, V_max, om_max):
    ...
    """
    ########## Code starts here ##########
    V_tilde = np.where(om > om_max, np.multiply(np.divide(om_max, om), V), V)
    V_tilde = np.where(om < -om_max, np.multiply(np.divide(-om_max, om), V), V_tilde)
    V_tilde = np.clip(V_tilde, -V_max, V_max)
    ########## Code ends here ##########
    return V_tilde
...
def compute_tau(V_tilde, s):
    ...
    """
    ########## Code starts here ##########
    tau = cumtrapz(np.divide(1., V_tilde), s, initial=0)
    ########## Code ends here ##########
    return tau
...
def rescale_om(V, om, V_tilde):
    ...
    """
    ########## Code starts here ##########
    om_tilde = np.multiply(np.divide(V_tilde, V), om)
    ########## Code ends here ##########
    return om_tilde
